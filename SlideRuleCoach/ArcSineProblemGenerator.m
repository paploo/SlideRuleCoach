...
@implementation ArcSineProblemGenerator
...
- (Problem *)nextWithDifficulty:(ProblemDifficulty)difficulty {
    ...
    double val = sin(randAngleInRadians);
    double angle = asin(val) * RADIAN_TO_DEGREE;
    ...
    NSString *numerator = [NSString stringWithFormat:@"asin(%@)", [defaultFormatter stringFromNumber:value]];
...
- (NSNumber *)scaleReadErrorForProblem:(Problem *)problem {
    double uAnswer = [ScaleParameterizer sinScaleParameterForValue:[[problem answer] doubleValue]];
    
    double uSubmitted = [problem submittedResult] ? [ScaleParameterizer sinScaleParameterForValue:[[problem submittedResult] doubleValue]] : 0.0;
